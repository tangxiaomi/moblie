...
* @Last Modified time: 2016-11-18 17:29:30
*/
...
var zhong = document.querySelectorAll('.zhong');

for(var i = 0; i < jia.length ; i++){
	jia[i].addEventListener('click',function (argument) {
		var txt = this.parentNode.children[1].children[0].value;
		txt++;
		this.parentNode.children[1].children[0].value = txt;
	})
}
for(var i = 0; i < jian.length ; i++){
	jian[i].addEventListener('click',function (argument) {
		var txt = this.parentNode.children[1].children[0].value;
		txt--;
		if(txt < 0){
			txt = 0;
